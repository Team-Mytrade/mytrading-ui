...
const drawHeader = <T extends object>(
  ...
  if (options.metadata?.length) {
    ...
    options.metadata.slice(0, 5).forEach((item) => {
      const text = `${item.label}: ${formatValue(item.value)}`;
      const width = Math.min(doc.getTextWidth(text) + 8, 54);

      doc.setFillColor(...BRAND.surface);
      doc.setDrawColor(...BRAND.border);
      doc.roundedRect(x, y - 5, width, 8, 2, 2, "FD");
      doc.setTextColor(...BRAND.ink);
      doc.setFontSize(7.5);
      doc.text(text, x + 4, y);
...
const drawFooter = (doc: jsPDF, pageWidth: number, pageHeight: number) => {
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text("Generated by My Trading", 14, pageHeight - 7);
...
export function exportListingPdf<T extends object>({
  ...
  const startY = metadata.length ? (subtitle ? 48 : 44) : subtitle ? 38 : 34;
