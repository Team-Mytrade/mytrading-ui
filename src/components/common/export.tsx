...
const drawHeader = <T extends object>(
  ...
  // Taller header band for breathing room
  doc.setFillColor(...BRAND.primary);
  doc.rect(0, 0, pageWidth, 20, "F");

  // Subtle accent strip under the band for depth
  doc.setFillColor(...BRAND.primaryDark);
  doc.rect(0, 20, pageWidth, 1, "F");

  if (logoDataUrl) {
    doc.setFillColor(255, 255, 255);
    doc.roundedRect(14, 4, 40, 12, 2, 2, "F");
    doc.addImage(logoDataUrl, "PNG", 16, 5.5, 36, 9);
  } else {
    doc.setTextColor(255, 255, 255);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(13);
    doc.text(options.companyName, 14, 13);
  }

  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(options.reportLabel.toUpperCase(), pageWidth - 14, 13, { align: "right" });
  ...
  if (options.subtitle) {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.5);
    doc.text(options.subtitle, 14, 38.5);
  }

  const generated = `Generated ${new Date().toLocaleString()}`;
  doc.setTextColor(...BRAND.muted);
  doc.setFontSize(8);
  doc.text(generated, pageWidth - 14, 32, { align: "right" });

  // Metadata shown as a light chip/banner row instead of plain text
  const filterSummary = getFilterSummary(options.metadata);
  if (filterSummary) {
    const y = options.subtitle ? 45 : 40;
    doc.setFillColor(...BRAND.surface);
    doc.roundedRect(14, y - 4.5, pageWidth - 28, 8, 1.5, 1.5, "F");
    doc.setTextColor(...BRAND.muted);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text(filterSummary, 18, y, { maxWidth: pageWidth - 36 });
  }
};
...
const drawFooter = (
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    // Light footer band instead of a bare line
    doc.setFillColor(...BRAND.surface);
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.text(`Generated by ${generatedBy}`, 14, pageHeight - 6);
...
export function exportListingPdf<T extends object>({
  ...
  const head = [
    [
      ...(includeSerialNumber ? [serialNumberLabel] : []),
  ...
  const startY = metadata.length ? (subtitle ? 56 : 52) : subtitle ? 46 : 42;
  ...
  body,
  startY,
  margin: { left: 14, right: 14, top: startY, bottom: 22 },   // ← add top: startY
  styles: {
    font: "helvetica",
    fontSize: 8.3,
