...
const PageBreadcrumb: React.FC<BreadcrumbProps> = ({
  ...
  return (
    <div className={`mb-6 flex flex-col gap-2 ${className}`.trim()}>
      <div className={`flex flex-col gap-3 rounded-2xl border border-gray-200 px-4 py-2 shadow-sm sm:px-5 ${contentClassName}`.trim()}>
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex min-w-0 flex-1 items-center gap-3 xl:relative xl:bottom-4">
            {showBackButton && (
              <button
                onClick={handleBack}
            ...
            <h2 className={`truncate text-[18px] font-semibold tracking-tight text-cyan-600 dark:text-white/90 sm:text-[20px] ${titleClassName}`.trim()}>
              {pageTitle}
            </h2>
          </div>

          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center xl:relative xl:bottom-4 xl:ml-auto xl:flex-nowrap xl:justify-end xl:gap-4">
            {inlineBreadcrumb && (
              <nav className={`min-w-0 max-w-full overflow-x-auto xl:w-[380px] 2xl:w-[420px] ${breadcrumbClassName}`.trim()}>
        ...
        {!inlineBreadcrumb && (
          <nav className={`max-w-full overflow-x-auto pl-11 ${breadcrumbClassName}`.trim()}>
