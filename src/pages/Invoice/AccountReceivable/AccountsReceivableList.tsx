...
const AccountsReceivableList: React.FC = () => {
    ...
    const [sortKey, setSortKey] = useState<keyof AccountsReceivable>("dueDate");
    ...
    const loadCustomerAR = async () => {
        ...
        try {
            ...
            setPage(1);
            ToasterService.success("Customer receivables loaded");
        } catch (err) {
    ...
    const loadCustomerOutstanding = async () => {
        ...
        try {
            ...
            setPage(1);
            ToasterService.success("Outstanding receivables loaded");
        } catch (err) {
            console.error("Error loading customer outstanding:", err);
            ToasterService.error("Failed to load customer outstanding");
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadOverdue = async () => {
        ...
        try {
            ...
            setRecords(Array.isArray(res.data) ? res.data : []);
            setPage(1);
            ToasterService.success("Overdue receivables loaded");
        } catch (err) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            ToasterService.success("Receivable deleted successfully");

            // Reload based on current view
            if (activeView === 'overdue') {
                await loadOverdue();
            } else if (activeView === 'customer' && selectedCustomerId) {
                await loadCustomerAR();
            } else if (activeView === 'outstanding' && selectedCustomerId) {
                await loadCustomerOutstanding();
            } else {
                setRecords([]);
            }
        } catch (err: any) {
            ToasterService.error(err.response?.data?.message || "Failed to delete receivable");
        } finally {
            setLoading(false);
        }
    };
    ...
    const exportPDF = async () => {
        ...
        try {
            ...
            let title = "All Receivables";
            if (activeView === 'overdue') title = "Overdue Receivables";
            else if (activeView === 'customer' && selectedCustomerId) {
                const customer = customers.find(c => c.id === selectedCustomerId);
                title = `Receivables for ${customer?.name || 'Customer'}`;
            } else if (activeView === 'outstanding' && selectedCustomerId) {
                const customer = customers.find(c => c.id === selectedCustomerId);
                title = `Outstanding Receivables for ${customer?.name || 'Customer'}`;
            }
            ...
            autoTable(doc, {
                head: [["Invoice", "Customer", "Due Date", "Amount", "Paid", "Balance", "Status"]],
                body: records.map(r => [
            ...
            if (customerSummary) {
                autoTable(doc, {
                    body: [
                        ["Total Outstanding:", `$${customerSummary.totalOutstanding.toFixed(2)}`],
                        ["Number of Items:", customerSummary.count.toString()]
                    ],
            ...
            doc.save(`Receivables_${activeView}_${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    const exportExcel = async () => {
        ...
        try {
            const data = records.map(r => ({
                ...
                "Balance": r.balance,
                "Status": r.accountsReceivableStatus
            }));

            // Add summary if available
            if (customerSummary) {
                data.push({} as any);
                data.push({ "Invoice #": "SUMMARY", "Customer": "Total Outstanding", "Balance": customerSummary.totalOutstanding } as any);
                data.push({ "Invoice #": "SUMMARY", "Customer": "Number of Items", "Balance": customerSummary.count } as any);
            }

            const ws = XLSX.utils.json_to_sheet(data);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Receivables");
            XLSX.writeFile(wb, `Receivables_${activeView}_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    const filtered = records.filter((r) => {
        ...
        const matchesStatus = selectedStatus ? r.accountsReceivableStatus === selectedStatus : true;
    ...
    const handleSort = (field: keyof AccountsReceivable) => {
        if (sortKey === field) {
    ...
    const SortIcon = ({ column }: { column: keyof AccountsReceivable }) => {
    ...
    const overdueCount = records.filter(r => r.accountsReceivableStatus === "OVERDUE").length;
    const paidCount = records.filter(r => r.accountsReceivableStatus === "PAID").length;
    ...
    return (
        <>
            ...
            <PageBreadcrumb
                ...
                showAddButton
                addButtonLabel="Add Receivable"
                onAddClick={() => navigate("/account-receivable/add")}
            />
            ...
            <div className="max-w-7xl mx-auto p-6">
                ...
                {/* Customer Selection and Actions Bar */}
                <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div className="flex flex-wrap gap-4 items-end">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                disabled={loading || exporting || !selectedCustomerId}
                                ...
                                title="Load all receivables for selected customer"
                            >
                                Customer AR
                            </button>
                            <button
                                ...
                                title="Load only outstanding receivables for selected customer"
                            >
                                Outstanding
                            </button>
                            <button
                    ...
                    {activeView !== 'all' && (
                        <div className="mt-3 flex items-center gap-2">
                            ...
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${activeView === 'overdue' ? 'bg-red-100 text-red-800' :
                                activeView === 'customer' ? 'bg-cyan-100 text-cyan-800' :
                                    'bg-purple-100 text-purple-800'
                                }`}>
                                {activeView === 'overdue' ? 'Overdue Receivables' :
                                    activeView === 'customer' ? 'Customer Receivables' :
                ...
                {/* Customer Summary - Only for outstanding view */}
                {customerSummary && (
                    <div className="mb-6 p-4 bg-purple-50 rounded-lg border border-purple-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                            <div className="bg-white p-3 rounded-lg border border-purple-100">
                                ...
                                <p className="text-xl font-bold text-purple-600">${customerSummary.totalOutstanding.toFixed(2)}</p>
                            </div>
                            <div className="bg-white p-3 rounded-lg border border-purple-100">
                            ...
                            <div className="bg-white p-3 rounded-lg border border-purple-100">
                                ...
                                <p className="text-xl font-bold text-purple-600">
                                    ${customerSummary.count > 0 ? (customerSummary.totalOutstanding / customerSummary.count).toFixed(2) : '0.00'}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <thead className="bg-gray-50">
                                <tr>
                                    {[
                                        ...
                                        { key: 'accountsReceivableStatus', label: 'Status' },
                                        { key: null, label: 'Actions' },
                                    ].map((col, idx) => (
                                        <th
                                            key={idx}
                                            onClick={() => !loading && !exporting && col.key && handleSort(col.key as keyof AccountsReceivable)}
                                        ...
                                        >
                                            <span className="flex items-center">
                                                {col.label}
                                                {col.key && <SortIcon column={col.key as keyof AccountsReceivable} />}
                                            </span>
                                        </th>
                                    ))}
                                </tr>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {paginated.length > 0 ? paginated.map((record) => (
                                    <tr
                                        ...
                                        onClick={() => !loading && !exporting && navigate(`/account-receivable/view/${record.id}`)}
                                    >
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <DocumentTextIcon className="h-5 w-5 text-gray-400 mr-2" />
                                                <span className="text-sm font-medium text-gray-900">{record.invoice.invoiceNumber}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(record.accountsReceivableStatus)}`}>
                                            ...
                                            <Menu as="div" className="relative inline-block text-left">
                                                ...
                                                <Menu.Items className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-md border border-gray-200 z-[100]">
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <button
                                                                onClick={() => navigate(`/account-receivable/view/${record.id}`)}
                                                                disabled={loading || exporting}
                                                    ...
                                                    <Menu.Item>
                                                        {({ active }) => (
                                                            <button
                                                                onClick={() => navigate(`/account-receivable/edit/${record.id}`)}
                                                                disabled={loading || exporting}
                                                                className={`${active ? "bg-gray-50" : ""} w-full text-left px-4 py-2 text-sm flex items-center gap-2 text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed`}
                                                            >
