...
const AccountsReceivableDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error("Error loading receivable details:", err);
                ToasterService.error("Failed to load receivable details");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handlePrint = () => {
        ...
        const statusColor = record.accountsReceivableStatus === "PAID" ? '#10b981' :
            record.accountsReceivableStatus === "PARTIALLY_PAID" ? '#f59e0b' :
                record.accountsReceivableStatus === "OVERDUE" ? '#ef4444' : '#3b82f6';

        printWindow.document.write(`
            <!DOCTYPE html>
            <html>
            <head>
                <title>Receivable Details - ${record.invoice.invoiceNumber}</title>
                <style>
                    body {
            ...
            <body>
                <div class="print-header">
                    <h1>Accounts Receivable Details</h1>
                ...
                <div class="print-footer">
                    Confidential Financial Record — Receivable Document<br>
        ...
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="text-center">
                    ...
                    <p className="text-red-500 font-medium">Receivable not found</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <>
            <PageMeta title={`Receivable - ${record.invoice.invoiceNumber}`} description="View receivable details" />
            <PageBreadcrumb pageTitle="Receivable Details" />
            ...
            <div className="max-w-7xl mx-auto p-6">
                ...
                <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            onClick={() => navigate(`/accounts-receivable/edit/${record.id}`)}
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">Edit Receivable</span>
                        </button>
                    </div>
                </div>
                ...
                <div ref={printRef}>
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        {/* Status Bar */}
                        <div className={`h-2 ${record.accountsReceivableStatus === "PAID" ? 'bg-green-500' :
                            record.accountsReceivableStatus === "PARTIALLY_PAID" ? 'bg-yellow-500' :
                                record.accountsReceivableStatus === "OVERDUE" ? 'bg-red-500' : 'bg-blue-500'
                            }`} />
                        ...
                        <div className="p-8">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8 pb-8 border-b border-gray-200">
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <h2 className="text-3xl font-bold text-gray-900 font-mono">{record.invoice.invoiceNumber}</h2>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(record.accountsReceivableStatus)}`}>
                                            {getStatusIcon(record.accountsReceivableStatus)}
                                            <span className="ml-2">{record.accountsReceivableStatus.replace('_', ' ')}</span>
                                        </span>
                                        {isOverdue && record.accountsReceivableStatus !== "PAID" && (
                                ...
                                <div className="bg-gray-50 rounded-lg p-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <p className="text-lg font-medium text-gray-900">{record.customer.name}</p>
                                        </div>
                                        {record.customer.email && (
                                            <div>
                                                <p className="text-sm text-gray-500">Email</p>
                                                <p className="text-lg font-medium text-gray-900">{record.customer.email}</p>
                                            </div>
                                        )}
                                        {record.customer.phone && (
                                            <div>
                                                <p className="text-sm text-gray-500">Phone</p>
                                                <p className="text-lg font-medium text-gray-900">{record.customer.phone}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-medium">
                                                        {record.amountPaid === 0 ? 'No payments received' :
                            ...
                            {record.createdDate && (
                                <div className="mt-6 pt-4 border-t border-gray-100 text-xs text-gray-400 flex justify-between">
                                    <div>Receivable ID: {record.id}</div>
