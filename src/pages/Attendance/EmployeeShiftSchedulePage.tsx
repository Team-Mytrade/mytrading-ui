    ...
    try {
      ...
      if (Array.isArray(empRes.data) && empRes.data.length > 0) {
        const mappedEmps = empRes.data.map((e: any) => ({
          id: Number(e.id),
          name: `${e.firstName || ''} ${e.lastName || ''}`.trim() || e.name || e.fullName || 'Employee',
          code: e.employeeCode || ''
        }));
  ...
  const columns: ColumnDef<AssignmentModel>[] = [
    {
      ...
      render: (row) => {
        ...
        const name = row.employeeName || emp?.name || 'Employee';
        return (
          <div>
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto pb-6 animate-in fade-in duration-200 mt-1 space-y-4">
        ...
        {/* Header Bar */}
        <div className="bg-white dark:bg-[#191919] rounded-xl shadow-2xs border border-gray-200/80 dark:border-transparent p-3.5 sm:p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-cyan-50 dark:bg-[#222222] text-cyan-700 dark:text-cyan-400 rounded-xl border border-cyan-200 dark:border-transparent">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              className="p-2 bg-gray-50 hover:bg-gray-100 dark:bg-[#222222] dark:hover:bg-[#2a2a2a] text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-transparent rounded-xl transition-all cursor-pointer shadow-2xs"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
          <div className="bg-white dark:bg-[#191919] p-4 rounded-xl shadow-2xs border border-gray-200/80 dark:border-transparent space-y-2">
            ...
            <select
              ...
              className="w-full py-2 px-3 bg-gray-50 dark:bg-[#222222] border border-gray-200 dark:border-transparent rounded-xl text-xs font-bold text-gray-800 dark:text-white focus:bg-white dark:focus:bg-[#191919] focus:ring-1 focus:ring-cyan-500 outline-none cursor-pointer"
            >
              {employees.map(e => (
          ...
          <div className="md:col-span-2 bg-white dark:bg-[#191919] p-4 rounded-xl shadow-2xs border border-gray-200/80 dark:border-transparent flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`p-2.5 rounded-xl border ${currentShiftInfo ? 'bg-emerald-50 dark:bg-[#222222] text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-transparent' : 'bg-amber-50 dark:bg-[#222222] text-amber-700 dark:text-gray-400 border-amber-200 dark:border-transparent'}`}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {currentShiftInfo ? (
                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-transparent dark:text-gray-300 dark:border-transparent text-[10px] font-bold uppercase">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                      Active
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 border border-amber-200 dark:bg-transparent dark:text-gray-400 dark:border-transparent text-[10px] font-bold uppercase">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                      Unassigned / Out of Shift
                    </span>
                  )}
                </div>
                ...
                {currentShiftInfo && (
                  <div className="flex items-center gap-3 mt-1 text-[11px] font-mono flex-wrap">
                    {currentShiftInfo.shiftCode && (
                      <span className="px-1.5 py-0.5 bg-cyan-50 dark:bg-transparent text-cyan-800 dark:text-gray-300 border border-cyan-200 dark:border-transparent rounded-md text-[10px] font-bold">
                        Code: {currentShiftInfo.shiftCode}
                      </span>
                    )}
                    {currentShiftInfo.startTime && (
                      <span className="text-cyan-700 dark:text-gray-300 font-semibold">
        ...
        <div className="bg-white dark:bg-[#191919] rounded-xl shadow-2xs border border-gray-200/80 dark:border-transparent p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Shift Roster</h3>
            <TableToolbar onRefresh={() => loadEmployeeSchedules(selectedEmployeeId)} />
          </div>
          <ReusableTable
            data={assignments}
            columns={columns}
