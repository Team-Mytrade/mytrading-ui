...
const StockLevelsManager: React.FC = () => {
    ...
    const exportExcel = () => {
        const ws = XLSX.utils.json_to_sheet(filtered.map(s => {
            ...
            return {
                ...
                'Product SKU': s.product?.productSku || "-",
    ...
    const filtered = stockLevels.filter((s) => {
        const matchesProduct = productFilter ? s.product?.productName === productFilter : true;
    ...
    const uniqueProducts = [...new Set(stockLevels.map(s => s.product?.productName).filter(Boolean))];
    ...
    const tableData: StockLevelRow[] = filtered.map((stock) => {
        ...
        return {
            ...stock,
            productName: String(stock.product?.productName ?? ""),
            productSku: String(stock.product?.productSku ?? ""),
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        {stock.product?.productSku && (
                            <div className="text-xs text-gray-500 mt-0.5">SKU: {stock.product.productSku}</div>
                        )}
                    </div>
                </div>
            ),
        ...
        <>
            ...
            <div className="w-full max-w-none px-0 sm:px-0 lg:px-0 py-8 space-y-6">
                                            ...
                                            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-medium text-gray-900">{selectedStock.product?.productName || "N/A"}</p>
                                                        {selectedStock.product?.productSku && (
                                                            <p className="text-xs text-gray-500 mt-1">SKU: {selectedStock.product.productSku}</p>
                                                        )}
                                                    </div>
                                                    <div>
                ...
                <DynamicPopup
                    ...
                    subText={deletingStock ? `Are you sure you want to delete stock level for "${deletingStock.product?.productName || "this item"}" at "${deletingStock.warehouse?.name || "this warehouse"}"? This action cannot be undone.` : "Are you sure you want to delete this stock level?"}
                    confirmLabel="Delete"
                    cancelLabel="Cancel"
