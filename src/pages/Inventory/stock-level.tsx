...
interface StockAdjustment {
    ...
    // The API returns a flat `productId` (number), NOT a nested `product`
    // object. `product` is kept here only as an optional fallback in case
    // some responses ever include it, but code should read `productId`
    // first — see handleEdit below.
    productId?: number;
    product?: Product;
    warehouse?: Warehouse;
    batch?: Batch;
...
const StockAdjustmentManager: React.FC = () => {
    ...
    const fetchBatchesByProduct = async (productId: number) => {
        try {
            ...
            console.error("Failed to load batches for products", err);
            setFilteredBatches([]);
            return [];
        }
    };
    ...
    const handleEdit = (adj: StockAdjustment) => {
        ...
        setForm({
            ...
            // The API sends a flat `productId`, not a nested `product` object.
            // Read `productId` first; fall back to `product?.id` only in case
            // an older/different response shape ever includes it.
            productId: (adj.productId ?? adj.product?.id)?.toString() || "",
        ...
        // Use adj.productId directly (the real field the API returns),
        // not adj.product?.id (which is normally undefined).
        if (adj.productId) {
            fetchBatchesByProduct(adj.productId);
        }
        if (adj.batch?.id) {
